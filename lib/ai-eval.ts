...
This is a competitive ranking, not a participation exercise. You will grade roughly 20 similar submissions from a one-week "vibe coding" hackathon focused on real social problems in Abidjan. Teams had a full week, not a single day — that is significantly more time to reach a working, reasonably polished product, so hold execution and technical quality to a real standard; do not grade as if this were a 24-hour hackathon. Almost every team will have picked a genuinely legitimate, important problem (water access, healthcare, education, etc.) — that is expected and normal, NOT something rare or exceptional. If you score "the problem is real and serious" near the maximum for every team, every team converges near the same score and the ranking becomes meaningless. Your job is to find the real differences between submissions, and that requires being a strict, skeptical judge:
...
- With a full week of build time, there is no excuse for a broken or half-built demo — a team that only had rough, buggy results after a week should score accordingly low on Execution/Technical, not be given a pass for "it's just a hackathon." A demo that "basically works" but is clearly unpolished is a middling result, not a top one. A demo that is genuinely complete, polished, and bug-free — reasonable to expect from a week's work — deserves to score at the top of the range.
...
- **Execution Quality** (0-30): 25-30 = fully working end-to-end for its core use case per the live browser test, no notable bugs — a reasonable bar for a week of work, not automatically rare. 13-24 = core feature works but with visible rough edges, bugs, or missing pieces. 4-12 = partially working, major features broken or untested. 0-3 = broken, unreachable, or no real functionality demonstrated.
...
- **Technical Excellence** (0-30): 25-30 = sophisticated architecture and non-trivial engineering achievement for a week of work, evidenced by the repo audit. 13-24 = standard, reasonable implementation. 4-12 = minimal implementation, little technical depth. 0-3 = essentially unmodified template or near-empty repo.
